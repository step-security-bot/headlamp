...
describe('PluginManager Test Cases', () => {
  ...
  test('Install Plugin', async () => {
    await PluginManager.install("https://artifacthub.io/packages/headlamp/test-123/appcatalog_headlamp_plugin", tempDir, "", mockProgressCallback);
    expect(mockProgressCallback).toHaveBeenCalledWith({ type: 'success', message: 'Plugin Installed' });
  });
  
  test('List Plugins', () => {
    PluginManager.list(tempDir, mockProgressCallback);
    // Assuming "app-catalog" plugin is in the list of plugins
    expect(mockProgressCallback).toHaveBeenCalledWith({ type: 'success', message: 'Plugins Listed', data: expect.any(Array) });
  ...
  test('No Update available for Plugin', async () => {
    ...
    expect(mockProgressCallback).toHaveBeenCalledWith({ type: 'error', message: 'No updates available' });
  });
  ...
  test('Update Plugin', async () => {
    ...
    packageJSON.artifacthub.version = `${semver.major(packageJSON.artifacthub.version)}.${semver.minor(packageJSON.artifacthub.version)}.${semver.patch(packageJSON.artifacthub.version)-1}`; // Reduce the version using semver
    // Write the updated package.json back to the file
    fs.writeFileSync(packageJSONPath, JSON.stringify(packageJSON, null, 2));
  
    await PluginManager.update("app-catalog", tempDir, "", mockProgressCallback);
    expect(mockProgressCallback).toHaveBeenCalledWith({ type: 'success', message: 'Plugin Updated' });
  });
  ...
  test('Uninstall Plugin', async () => {
    ...
    expect(mockProgressCallback).toHaveBeenCalledWith({ type: 'success', message: 'Plugin Uninstalled' });
