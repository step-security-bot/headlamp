...
yargs(process.argv.slice(2))
  ...
  .command(
    'install [URL]',
    'Install a plugin from the specified URL',
    yargs => {
      yargs
        .positional('URL', {
  ...
  .command(
    ...
    async argv => {
      ...
      // const progressCallback = json ? null : (data)=>{console.log(data.type,":",data.message,data.data)}; // Use console.log for logs if not in quiet mode
